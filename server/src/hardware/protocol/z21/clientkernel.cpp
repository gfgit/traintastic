...
void ClientKernel::send(const Message& message, bool wantReply, int customRetryCount)
{
    ...
    if(wantReply)
    {
      ...
      if(request.reply.header != MessageReplyType::noReply)
      {
        ...
        else
        {
          ...
          switch (request.reply.getPriority())
          {
          case MessageReplyType::LowPriority:
            request.retryCount = 1;
            break;

          default:
          case MessageReplyType::Normal:
            request.retryCount = 2;
            break;

          case MessageReplyType::Urgent:
...
std::optional<ClientKernel::PendingRequest> ClientKernel::matchPendingReplyAndRemove(const Message &message)
{
  ...
  for(auto request = m_pendingRequests.begin(); request != m_pendingRequests.end(); request++)
  {
    ...
    if(message.header() == LAN_X)
    {
      ...
      if(request->reply.hasFlag(MessageReplyType::CheckDb0))
      {
    ...
    if(request->reply.hasFlag(MessageReplyType::CheckAddress))
    {
      uint16_t address = 0;
      switch (message.header())
      {
    ...
    if(request->reply.hasFlag(MessageReplyType::CheckSpeedStep))
    {
      if(message.header() == LAN_X
