...
MessageReplyType getReplyType(const Message &message)
{
  ...
  case LAN_GET_HWINFO:
  {
    ...
    reply.setPriority(MessageReplyType::Priority::LowPriority);
    return reply;
  }
  case LAN_GET_BROADCASTFLAGS:
  ...
  case LAN_GET_TURNOUTMODE:
  {
    MessageReplyType reply;
    reply.header = message.header();
    reply.setFlag(MessageReplyType::CheckAddress);
  ...
  case LAN_X:
  {
    ...
    case 0x21:
    {
      ...
      reply.header = LAN_X;
      reply.setPriority(MessageReplyType::Priority::LowPriority);
      reply.setFlag(MessageReplyType::CheckDb0);
    ...
    case LAN_X_SET_LOCO:
    {
      ...
      else
      {
            ...
            setLocoDrive.db0 >= 0x10 && setLocoDrive.db0 <= 0x13)
        {
          ...
          reply.setFlag(MessageReplyType::CheckSpeedStep);
        }
        else if(const auto& setLocoFunction = static_cast<const LanXSetLocoFunction&>(message);
                 setLocoFunction.db0 == 0xF8)
        {
