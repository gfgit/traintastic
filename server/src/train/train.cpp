...
void Train::updateAcceleration()
{
  ...
  double brakingMax = 0.0;

  bool accelerationSet = false;
  bool brakingSet = false;
  ...
  if(!vehicles->empty() && powered)
  {
    for(const auto& item : m_poweredVehicles)
    {
      ...
      if(!brakingSet || itemBrakingRate < brakingMax)
      {
        brakingSet = true;
        brakingMax = itemBrakingRate;
      }
    }
  }
  ...
  if(brakingSet && !almostZero(brakingMax))
  {
    if(brakingRate > brakingMax)
      brakingRate = brakingMax;
    Attributes::setMax(brakingRate, brakingMax);
  }
}
